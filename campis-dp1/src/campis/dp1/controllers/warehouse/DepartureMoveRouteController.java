...
//    private ArrayList<Integer> numList = new ArrayList<>();
    ...
    private void saveDepartureMove() throws IOException{
        ...
        for (int i=0; i<batchesList.size();i++) {
            ...
            WarehouseMove move = new WarehouseMove(currentTimestamp, ContextFX.getInstance().getId_User(), batch.getQuantity(), batchesList.get(i).getZone().getId_zone(), 2, 2, id_warehouse,batch.getId_batch());
            batch.setType_batch(3);
            Query query = session.createQuery(
                "update Batch set type_batch = :t_batch " + " where id_batch = :batchId ");
            query.setParameter("t_batch", 4);
